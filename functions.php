...
class front_nav_walker extends Walker_Nav_Menu
{
						...
						$id=0)
	{
		...
		if ($depth == 0) {
			$output.='<div id="menu-item-wrap-'. $item->ID .'"class="table-wrap">';
		}
...
if (!function_exists('marki_setup')) {
	function marki_setup() {
		...
		//Register the menus.
		register_nav_menu('main-menu',__('Primary Navigation Menu'));
		...
		add_theme_support('custom-background',apply_filters('marki_custom_background_args',array('default-color'=>$default_color,'default-attachment'=>'fixed')));
...
function marki_widgets_init() {
	register_sidebar( array('name' => __( 'Widget Space'),'id' => 'sidebar-1','description' => __( 'Add widgets here!'),'before_widget' => '<aside id="%1$s" class="widget %2$s">','after_widget' => '</aside>', 'before_title' => '<h2 class="widget-title">', 'after_title' => '</h2>'));
}
...
function marki_post_nav_background() {
	...
	wp_add_inline_style( 'marki-style', $css );
}
 ...
 */
function marki_search_form_modify( $html ) {
